//
// Created by Nicho on 31/12/2020.
//
#pragma once
...
#include "../../include/Graphs/EagerPrim.h"


TEST_CASE("Eager Prim's algorithm on a connected graph") {
    ...
    wtl::EagerPrim eagerPrim(graph);
    REQUIRE(eagerPrim.getCount() == 1);
    REQUIRE(eagerPrim.getMSTWeight(eagerPrim.belongs(6)) == 72);
    REQUIRE_THAT(eagerPrim.edges(eagerPrim.belongs(0)), Catch::Matchers::UnorderedEquals(mstRef));
}
...
TEST_CASE("Eager Prim's algorithm on a disconnected graph") {
    ...
    SECTION("First MST in MSF") {
        ...
        REQUIRE_THAT(eagerPrim.vertices(eagerPrim.belongs(3)),
                     Catch::Matchers::UnorderedEquals(std::vector<std::size_t>{0, 1, 2, 3}));
        REQUIRE_THAT(eagerPrim.edges(eagerPrim.belongs(0)),
    ...
    SECTION("Second MST in MSF") {
        ...
        REQUIRE_THAT(eagerPrim.vertices(eagerPrim.belongs(4)),
                     Catch::Matchers::UnorderedEquals(std::vector<std::size_t>{4, 5}));
        REQUIRE_THAT(eagerPrim.edges(eagerPrim.belongs(5)),
                     Catch::Matchers::UnorderedEquals(mstRef));
    }

    SECTION("Last 2 MST in MSF") {
        REQUIRE(eagerPrim.getMSTWeight(eagerPrim.belongs(6)) == 0);
        REQUIRE(eagerPrim.getMSTWeight(eagerPrim.belongs(7)) == 0);
        REQUIRE_THAT(eagerPrim.vertices(eagerPrim.belongs(6)),
                     Catch::Matchers::Equals(std::vector<std::size_t>{6}));
        REQUIRE(eagerPrim.edges(eagerPrim.belongs(7)).size() == 0);
    }
